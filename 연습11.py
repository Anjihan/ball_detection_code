#예제 10.12 cv2.CamShift + cv2.KalmanFilter 물체 추적 -> 코드실행이 안되는데 아무나 돌려주세요 ㅜㅜ
import cv2
import numpy as np
...
while True:
  ...
  if mouse_status == 3:
    ...
    hist_roi = cv2.calcHist([hsv_roi], [0], mask_roi, [16],
                            [0, 180])
  ...
  #4-1: Kalman filter initialize
    ...
    KF.errorCovPost = np.eye(4, dtype= np.float32)    # PO = I

    x, y, w, h = track_window
    cx = x + w / 2
    cy = y + h / 2
    KF.statePost = np.array([[cx], [cy], [0.], [0.]],
                            dtype = np.float32)
    tracking_start = True
...
#4-3: CamShift tracking
    ...
    track_box, track_window = cv2.CamShift(backP, track_window,
                                      term_crit)
...
#4-5
    ...
    track_box2 = (float(cx2), float(cy2),
                  track_box[1], track_box[2])
